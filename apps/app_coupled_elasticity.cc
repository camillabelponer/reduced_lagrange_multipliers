                ...
                while (timestep < (tEnd - tIni))
                  {
                    ...
                    // Vector<double> coupling_pressure; // TODO put here so i
                    // can save when i don't update it to prevent jumps every
                    // iterSampling we aso perform the 3D
                    if (timestep > couplingStart &&
                        iter % couplingSampling == 0)
                      {
                        ...
                        timer.leave_subsection();
                        // problem3D.compute_coupling_pressure(); // check that
                        // it's done automatically at each timestep

                        if (coupling_mode == 0)
                          {
                            Vector<double> coupling_pressure(
                              problem3D.coupling_pressure);
                            // TODO metti fuori dal ciclo questo
                            // TODO add baseline_pressure
                            // coupling_pressure = problem3D.coupling_pressure;
